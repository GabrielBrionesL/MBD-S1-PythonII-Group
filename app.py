...
selection = st.sidebar.radio('Go to', ['Home','Conclusions', 'Technical Report','Simulator', 'Auto ML - Upload', 'Auto ML - EDA', 'Auto ML - Modelling', 'Auto ML - Download'])
...
if selection == "Auto ML - Modelling":
    if df is not None:
        st.title("Modeling with PyCaret")
        ...
        if st.button('Run Modeling'):
            ...
            # Visualize RMSE of each model using Plotly
            fig = px.bar(compare_df, x='Model', y='RMSE', title='RMSE of Each Model')
            st.plotly_chart(fig)

            # Visualize MAE of each model using Plotly
            fig_mae = px.bar(compare_df, x='Model', y='MAE', title='MAE of Each Model')
            st.plotly_chart(fig_mae)
    else:
        st.warning("Please upload a dataset first.")

if selection == "Auto ML - Download":
    if os.path.exists('best_model.pkl'):
        with open('best_model.pkl', 'rb') as f:
            st.download_button('Download Model', f, file_name="best_model.pkl")
    else:
        st.warning("No model available. Please run modeling first.")
